...
for d in $dapil; do
  for p in $partai; do
    rm -f result/caleg/caleg_provinsi_$d_$p.json 2>/dev/null
    getJson https://infopemilu.kpu.go.id/pileg2019/pencalonan/pengajuan-calon/$d/$p/dct?_=$(date +%s) result/caleg/caleg_provinsi_$d_$p.json
  done
done
